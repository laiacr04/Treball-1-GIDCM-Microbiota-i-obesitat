...
#Visualització inicial: boxplot per taxó
...
#Comprovar cada fila suma 1 i normalitzar
row_sums <- rowSums(microbiota)
microbiota_prop <- microbiota / row_sums
summary(rowSums(microbiota_prop))  # totes les files sumen 1
...
#Visualització multivariant de la composició segons obesitat
...
#(loadings): quins taxons contribueixen més a cada component
...
#Gràfic de les càrregues del primer component principal
...
#Càlcul de les coordenades OLR
microbiota_olr <- coordinates(as.matrix(microbiota_prop), basis = sbp_basis(SBP))
microbiota_olr_df <- as.data.frame(microbiota_olr)
head(microbiota_olr_df)

#Representació de les coordenades OLR
boxplot(microbiota_olr_df, main = "Distribució de les coordenades OLR (microbiota)",
